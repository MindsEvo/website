 ...
 *   me:sci:tpl:{id}          → per-template state (mastery, cooldown, history)
 *   me:sci:coverage          → coverage window (last 20 attempt type strings)
 *   me:sci:global_attempts   → total attempt counter across all templates
 *   me:sci:upload_queue      → pending server events
 */

var SciEngine = (function () {
  ...
  function selectSessionTemplates(level, templates) {
    console.warn('[SciEngine] selectSessionTemplates() is deprecated — use getSessionTemplates()');
  ...
  //   SciEngine.setEventsEndpoint('http://localhost:8787/api/events')
